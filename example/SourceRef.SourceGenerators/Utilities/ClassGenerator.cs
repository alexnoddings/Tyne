...
internal static class ClassGenerator
{
    ...
    public static SourceText GenerateClass(string className, IEnumerable<SourceInfo> sources, Func<SourceInfo, string> fieldFunc)
    {
        ...
        classBuilder.AppendLine("namespace Tyne.SourceRef");
        classBuilder.AppendLine("{");
        classBuilder.AppendLine($"    internal static class {className}");
        classBuilder.AppendLine("    {");
        ...
        foreach (var sourceInfo in sources.OrderBy(type => type.Identifier))
        {
            ...
            classBuilder.AppendLine($"        public const string {sourceInfo.IdentifierNormalised} = \"{fieldValue}\";");
        }

        classBuilder.AppendLine("    }");
        classBuilder.AppendLine("}");

        var classContents = classBuilder.ToString();
